...
func Register(userCollection *mongo.Collection) gin.HandlerFunc{
	return func(ctx *gin.Context){
		...
		InsertedID, err := data.Register(userCollection, user)
		if err != nil {
			ctx.JSON(http.StatusInternalServerError, gin.H{
		...
		ctx.JSON(http.StatusAccepted, gin.H{"message": "User registered successfully", "user": user, "id": InsertedID})
	}
}
...
func Login(userCollection *mongo.Collection) gin.HandlerFunc{
	return func(ctx *gin.Context){
		...
		// Call the user service to add the user
		token, err := data.Login(userCollection, user)
		...
		ctx.JSON(200, gin.H{"message": "User logged in successfull", "token": token})
	} 
}
