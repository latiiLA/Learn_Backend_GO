...
func (tc *TaskController) Create(c *gin.Context){
	...
	if userID == "" {
		c.JSON(http.StatusBadRequest, ErrorResponse{Message: "userID not found in context"})
		return
	}
	...
	c.JSON(http.StatusOK, SuccessResponse{Message: "Task created successfully"})
}
...
func (tc *TaskController) Fetch(c *gin.Context){
	...
	// Convert userID from hex string to primitive.ObjectID
	...
	tasks, err := tc.TaskUsecase.GetTasksByUserID(c, objectID)
	if err != nil{
		c.JSON(http.StatusInternalServerError, ErrorResponse{Message: err.Error()})
		return
	}
